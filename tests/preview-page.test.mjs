...
const { stageVars, injectHtmlVars, addNoFx, addBaseHref, injectStyle, buildPlayPage } = mod;
...
<body><div class="stage"><p class="fx-up" data-stage="1">文案</p></div></body>
</html>
`;
...
describe('预览放映页 · 纯函数', () => {
  ...
  test('buildPlayPage: 自包含(无外链/无外站脚本)且含全部张次', () => {
    const page = buildPlayPage({
      topic: 'T', slides: [
        { id: '01', name: '01.html', copy: '01.html', copyNofx: '01.nofx.html', duration: 6.3, lastStage: 3.2, clauses: [{ stage: 1, start: 0, text: '甲' }] },
        { id: '02', name: '02.html', copy: '02.html', copyNofx: '02.nofx.html', duration: 5, lastStage: 2, clauses: [] },
      ], noFxNote: '注意 X',
    });
...
describe('预览放映页 · 落到磁盘', () => {
  ...
  test('没有 timings.json → 照常出页, 但明确警告时序不是成片的', () => {
    ...
    assert.ok(fs.readFileSync(path.join(proj, 'preview', 'play', 'index.html'), 'utf8').includes('占位延迟'));
  });
