...
**字幕切换不重叠(2026-09-17 修复)**:每句字幕的淡入与淡出都强制在它自己的窗口 `[本句开口, 下句开口]` 内完成, 在切换点归零 —— 任意时刻只有一条字幕可见。历史 bug:淡出曾越界到下一条的窗口(fade 取窗口的 15%), 造成 0.5–0.9s 的两条不同字幕同时可见, 视觉上是"重影/叠字", 极易被误判成"字幕元素被创建了两次"或字体问题。capture 现在生成字幕时会做窗口自检, 发现重叠或窗口过短会直接告警。
