...
- Hard ceiling of 60 characters of voiceover for a single slide; over that means the slide is doing the work of two, so split it. In subtitle scenarios, prefer one line per sentence — the pill fits **≈24 CJK characters per line at 1080 width, ≈33 at 1920** (it scales with the canvas; one-line craft guideline ≈18; capture burns subtitles as one wrapped block: two lines still read fine, `plan-timings` warns at three or more); when bilingual, the second line (text2) is ≤60 characters.
